...
PROGRAM SIR
    IMPLICIT NONE 
    REAL :: beta=0.5, gamma=0.1111111, mu=0.100
    REAL :: A=1449401, N= 1449401, h=0.001, tfinal=60
    REAL :: S0, I0, R0
    REAL :: S, I, R
    INTEGER :: eNe
    REAL, DIMENSION(10) :: const
    REAL, DIMENSION(:), allocatable :: o,p,q 
    INTEGER :: j
    S0 =1446093/N
    I0 = 1885/N
    R0 = 1423/N
    eNe=INT(CEILING(tfinal/h)+1 )
    const=(/beta,gamma,mu,A,N,h,tfinal,S0,I0,R0/)
    ALLOCATE(o(eNe))
    ALLOCATE(p(eNe))
    ALLOCATE(q(eNe))
    o(1)=S0
    p(1)=I0
    q(1)=R0
    OPEN(12,file='Resultados_SIR.txt')
    OPEN(13,file="S.txt")
    OPEN(14,file="I.txt")
    OPEN(15,file="R.txt")    
    DO j=2, eNe 
